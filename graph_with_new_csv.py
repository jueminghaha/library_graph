...
def parse_csv_file(file_path):
    ...
    with open(file_path, newline='', encoding='utf-8') as csvfile:
        ...
        next(reader)  # 跳过标题行
        for row in reader:
            cited = clean_title(row[0])
            citing = clean_title(row[1])
            citations.append((citing, cited))
    return citations

def build_directed_graph(entries, center_article_id, center_article_year, csv_citations):
    G = nx.DiGraph()
    ...
    # 添加 CSV 文件中的引用关系
    for citing, cited in csv_citations:
        if cited not in G.nodes():
            continue  # 跳过不存在的被引用文章
        if citing not in G.nodes():
            G.add_node(citing, year=None)
...
if __name__ == "__main__":
    ...
    csv_citations = parse_csv_file(csv_file_path)
    graph = build_directed_graph(entries, center_article_id, center_article_year, csv_citations)
